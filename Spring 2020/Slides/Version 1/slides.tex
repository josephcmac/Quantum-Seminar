...
$$
f_{t,s}^{\delta}\left( |\varphi\rangle \right) = \max_{| \psi \rangle \in \text{span}\left(B_{t,s}^{\delta}\right)\otimes \mathcal{H}_E}  | \langle \psi | \varphi \rangle |^2.
$$
\end{frame}
...
$$
\varepsilon_q^{\delta} :=\max_{\mathcal{H}_E} \max_{|\varphi_{AE}\rangle \in \mathcal{H}_A\otimes \mathcal{H}_E} \sum_{(t,s)\in\mathcal{T}\times\mathcal{S}} P_{T,S}(t,s) \sqrt{1 -  f_{t,s}^{\delta} \left(|\varphi_{AE}\rangle \right)}.
$$
\end{frame}
